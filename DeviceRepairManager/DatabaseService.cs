...
namespace DeviceRepairManager
{
    public class DatabaseService
    {
        ...
        private void SeedInitialData(SQLiteConnection connection)
        {
            ...
            var workOrdersData = new (string Status, string Priority, string Notes)[]
            {
                ("Folyamatban", "Közepes", "Első munkalap."),
                ("Befejezve", "Magas", "Második munkalap."),
                ("Folyamatban", "Alacsony", "Harmadik munkalap."),
                ("Árajánlat", "Közepes", "Negyedik munkalap.")
            };

            foreach (var wo in workOrdersData)
            {
                cmd.CommandText = @"
                    INSERT INTO WorkOrders (CreationDate, Status, Priority, Notes, HoursWorked, RequiresApproval, CreatedBy)
                    SELECT DATE('now'), @Status, @Priority, @Notes, 0, 0, 'Seed'
                    WHERE NOT EXISTS (
                        SELECT 1 FROM WorkOrders WHERE Status = @Status AND Priority = @Priority AND Notes = @Notes
                    );
                ";
                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@Status", wo.Status);
                cmd.Parameters.AddWithValue("@Priority", wo.Priority);
                cmd.Parameters.AddWithValue("@Notes", wo.Notes);
                cmd.ExecuteNonQuery();
            }

            var workOrderIds = new List<int>();
            cmd.CommandText = "SELECT WorkOrderId FROM WorkOrders ORDER BY WorkOrderId;";
            using (var reader = cmd.ExecuteReader())
            {
                while (reader.Read())
                {
                    workOrderIds.Add(Convert.ToInt32(reader["WorkOrderId"]));
                }
            }
            ...
            {
                var r = repairs[i];
                int workOrderId = i < workOrderIds.Count ? workOrderIds[i] : (int?)null ?? 0;

                cmd.CommandText = @"
                    INSERT INTO Repairs (DeviceId, CustomerId, TechnicianId, WorkOrderId, StartDate, Status, Description, CustomerApproved, RepairCount, IsUnderRepair)
                    SELECT @DeviceId, @CustomerId, @TechnicianId, @WorkOrderId, DATE('now'), @Status, @Description, 0, @RepairCount, 1
                    WHERE NOT EXISTS (
                        SELECT 1 FROM Repairs WHERE DeviceId = @DeviceId AND Description = @Description
                    );
                ";
                cmd.Parameters.Clear();
